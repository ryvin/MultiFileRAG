...
class ProcessingStage:
    """Data class for document processing stage information."""
    doc_id: str
    stage: str
    status: str
...
class DocumentProcessor:
    ...
    # NOTE: This method is complex by design as it orchestrates the entire document processing pipeline.
    # A future refactoring should break this into smaller methods for each processing stage.
    # Current issues: too many local variables (32/15), too many branches (20/12), too many statements (77/50)
    async def process_document(self, file_path: str) -> bool:
        ...
        try:
            ...
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="start", status="in_progress"
            ))

            # Step 1: Chunking
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="chunking", status="in_progress"
            ))
            chunks = await self.rag.chunk_document(file_path)
            if not chunks:
                await self._record_processing_stage(ProcessingStage(
                    doc_id=doc_id, stage="chunking", status="failed", error="No chunks generated"
                ))
                await self._record_processing_stage(ProcessingStage(
                    doc_id=doc_id, stage="overall", status="failed", error="Chunking failed"
                ))
                return False
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="chunking", status="completed",
                metadata={"chunk_count": len(chunks)}
            ))

            # Step 2: Entity Extraction
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="entity_extraction", status="in_progress"
            ))
            entities_by_chunk = {}
            relationships = []
            ...
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="embedding_generation", status="in_progress"
            ))
            embeddings_by_chunk = {}
            ...
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="vector_storage", status="in_progress"
            ))
            vector_keys = []
            ...
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="graph_storage", status="in_progress"
            ))
            ...
            # Step 6: Indexing
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="indexing", status="in_progress"
            ))
            ...
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="indexing", status="completed"
            ))

            # Record overall completion
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="overall", status="completed"
            ))

            return True

        except IOError as e:
            logger.error("IO error processing document %s: %s", file_path, e)
        except ValueError as e:
            logger.error("Value error processing document %s: %s", file_path, e)
        # NOTE: General exception handler is needed as a fallback to catch any unexpected errors
        # that weren't caught by the specific handlers above.
        except Exception as e:
            logger.error("Unexpected error processing document %s: %s", file_path, e)
            await self._record_processing_stage(ProcessingStage(
                doc_id=doc_id, stage="overall", status="failed", error=str(e)
            ))
            return False

    async def _record_processing_stage(self, stage_info: ProcessingStage):
        ...
        try:
            ...
            key = f"processing:{stage_info.doc_id}:{stage_info.stage}"
            value = {
                "doc_id": stage_info.doc_id,
            ...
            if stage_info.error:
                value["error"] = stage_info.error

            if stage_info.metadata:
                value["metadata"] = stage_info.metadata

            await self.cache.set(key, json.dumps(value))

            # Log processing stage status
            logger.info(
                "Document %s, stage %s: %s",
                stage_info.doc_id,
                stage_info.stage,
                stage_info.status
            )
            ...
            logger.error("Value error recording processing stage: %s", e)
        except IOError as e:
            logger.error("IO error recording processing stage: %s", e)
        # NOTE: General exception handler is needed as a fallback to catch any unexpected errors
        # that weren't caught by the specific handlers above.
        except Exception as e:
            logger.error("Unexpected error recording processing stage: %s", e)
...
class QueryProcessor:
    ...
    async def process_query(self, query: str, mode: str = "hybrid") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Value error processing query: %s", e)
        except KeyError as e:
            logger.error("Key error processing query: %s", e)
        # NOTE: General exception handler is needed as a fallback to catch any unexpected errors
        # that weren't caught by the specific handlers above.
        except Exception as e:
            logger.error("Unexpected error processing query: %s", e)
            return {
                "query": query,
                "mode": mode,
                "error": str(e),
    ...
    async def _vector_search(self, _: str, query_embedding: List[float]) -> List[Dict[str, Any]]:
        """
    ...
    async def _graph_search(self, _: str, query_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
