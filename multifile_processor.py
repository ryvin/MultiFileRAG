...
def extract_text_from_pdf(file_path):
    ...
    try:
        ...
        # If PyPDF2 didn't extract much text, try textract as a fallback
        ...
        with open(file_path, "rb") as pdf_file:
            ...
            if info:
                text_content = f"PDF Metadata:\n"
                for key, value in info.items():
                    if key.startswith('/'):
                        key = key[1:]
                    text_content += f"{key}: {value}\n"
                text_content += f"Number of pages: {len(pdf_reader.pages)}\n\n"
...
def process_file(file_path):
    ...
    if file_extension in ['.pdf']:
        ...
        # For other file types, try textract
        try:
